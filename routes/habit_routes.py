# Importamos utilidades de fechas:
# - date: para trabajar con fechas (sin hora)
# - datetime: para parsear texto a fecha
# - timedelta: para sumar/restar días (ej: últimos 7 días)
from datetime import date, datetime, timedelta

# Importamos helpers de Flask para:
# - Blueprint: agrupar rutas
# - flash: mostrar mensajes al usuario
# - redirect/url_for: redirecciones
# - render_template: renderizar HTML
# - request/session: leer datos del formulario y usuario logueado
from flask import Blueprint, flash, redirect, render_template, request, session, url_for

# Importamos la instancia de base de datos SQLAlchemy compartida
from extensions import db

# Importamos nuestros modelos nuevos de hábitos
from models.habit import Habit
from models.habit_log import HabitLog

# Decorador para proteger rutas (si no hay login, redirige a /auth/login)
from utils.auth import login_required

# Creamos el Blueprint de hábitos.
# No usamos url_prefix para respetar exactamente rutas como /habits y /dashboard.
...
def _parse_date_or_today(raw_date: str | None) -> date:
    ...
    """
    # Si no viene nada (?date=...), usamos hoy.
    if not raw_date:
        return date.today()
